...
public class RingTorus 
   implements Comparable<RingTorus> {
...
/**
   * Compares two volumes for two RingTorus objects.
   *
   * @param a the object getting compared to
   *
   * @return returns if the object is greater than less 
   * than or equal to other object
   */
   public int compareTo(RingTorus a) {
      if (Math.abs(this.volume() - a.volume()) < .000001) {
         return 0;
      }
      else if (this.volume() - a.volume() > 0) {
         return 1;
      }
      else {
         return -1;
